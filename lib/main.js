...
const { detectVirtualEnv, detectPipEnv, sanitizeConfig } = require("./utils");
...
class PythonLanguageClient extends AutoLanguageClient {
  ...
  async startServerProcess(projectPath) {
    ...
    if (python.indexOf('$PIPENV_PATH') !== -1 && venvPath) {
      if (venvPath) {
        python = python.replace('$PIPENV_PATH', venvPath);
      } else {
        python = python.replace('$PIPENV_PATH', '');
      }
    }
